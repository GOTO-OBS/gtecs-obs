...
CREATE TABLE IF NOT EXISTS `goto_obs`.`mpointings` (
  ...
  `num_repeats` INT NOT NULL COMMENT 'can be -1 for infinite repeats',
  `num_remain` INT NOT NULL COMMENT '-1 for infinite repeats',
  ...
  INDEX `remain_idx` (`num_remain` ASC),
...
CREATE TABLE IF NOT EXISTS `goto_obs`.`exposures` (
  ...
  `unitTelescope` INT NULL COMMENT 'bit mask to allocate to individual UTs. NULL means send to all',
...
BEGIN
	IF (NEW.ts <> OLD.ts AND NEW.status NOT IN ('running', 'pending')) THEN
		IF NEW.`repeatNum` >= 0 THEN
			UPDATE `mpointings` SET `num_remain` = `num_remain` - 1, `rank` = `rank` + 10, `scheduled` = 0 WHERE (NEW.`mpointings_rpID` = `mpointings`.`rpID`);
		ELSE
			UPDATE `mpointings` SET `scheduled` = 0 WHERE (NEW.`mpointings_rpID` = `mpointings`.`rpID`);
        END IF;
	ELSEIF (NEW.ts <> OLD.ts AND NEW.status = 'pending') THEN
...
BEGIN
	IF (NEW.ts <> OLD.ts AND NEW.repeats_repeatID is not NULL) THEN
        IF (SELECT `repeatNum` FROM `repeats` WHERE (NEW.repeats_repeatID = `repeats`.`repeatID`)) >= 0 THEN
			UPDATE `repeats` SET `status` = NEW.status WHERE (NEW.repeats_repeatID = `repeats`.`repeatID`);
		ELSE
			IF NEW.status IN ('running', 'pending') THEN
				UPDATE `repeats` SET `status` = NEW.status WHERE (NEW.repeats_repeatID = `repeats`.`repeatID`);
            ELSE
				UPDATE `repeats` SET `status` = 'upcoming' WHERE (NEW.repeats_repeatID = `repeats`.`repeatID`);
			END IF;
        END IF;
    END IF;
END$$
